...
const TopOneHundredTab = (props: TopOneHundredTabProps) => {
  ...
  const {videosTop100, page, hasMore, loadingTopTab} = useReduxSelector<any>(
  ...
  useEffect(() => {
    if (hasMore && currentIndex >= videosTop100.length - 2) {
      ...
      dispatch(
        getVideosTopAction({skip, take: TAKE, orderBy: {createdAt: 'desc'}}),
      );
    }
  }, [currentIndex, dispatch, hasMore, page, videosTop100.length]);

  useEffect(() => {
    if (videosTop100.length === 0) {
      return;
    }
  ...
  const handleSingleTap = (item: VideoItemType, index: number) => {
    dispatch(
      getVideoCommentsAction({videoId: item?.id, userId: item?.user.id}),
    );
  ...
  const handleDoubleTap = (
    ...
    if (existingLike) {
      dispatch(
        deleteLikeAction({id: existingLike.id, userId: videoOwnerId, videoId}),
      );
    } else {
      const likesBodyData = {
        dataLike: {
