...
export const apiLikesFetcher = async (userId: number, videoIds: number[]) => {
  const _fetchLike = async (videoId: number) => {
    const { data } = await api.get<LikeResponse>(
      `/api/reactions?where[typeField]=Like&where[video][id]=${videoId}&where[user][id]=${userId}`,
    );
    return data;
  };

  const likes = await Promise.allSettled(videoIds.map((videoId) => _fetchLike(videoId)));
