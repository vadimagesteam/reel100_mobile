...
import { VideoItemType } from '../../../../redux/CameraRedux/types.ts';
import { deleteLikeAction, getLikesAction, setLikeAction } from '../../../../redux/LikesRedux/likesAction.tsx';
...
interface FullVideoModalProps {
  setModalVideo: (val: VideoItemType | null) => void
  modalVideo: VideoItemType | null
}

const FullVideoModal = ({ modalVideo, setModalVideo }: FullVideoModalProps) => {
  ...
  // Load likes when modalVideo changes
  useEffect(() => {
    if (modalVideo?.id && modalVideo.user?.id) {
  ...
  }, [modalVideo, user, likesData, dispatch, tapX, tapY, scale, opacity]);

  // Gestures
  const singleTapGesture = useMemo(
    () => Gesture.Tap().maxDuration(250).onEnd(() => runOnJS(handleSingleTap)()),
    [handleSingleTap]
  );
  const doubleTapGesture = useMemo(
    () => Gesture.Tap().numberOfTaps(2).onEnd(event => {
      if (event && modalVideo?.id && modalVideo.user?.id) {
        runOnJS(handleDoubleTap)(event.x, event.y);
      }
    }),
    [handleDoubleTap, modalVideo]
  );
  ...
  const hasHlsUrl = !!modalVideo?.file?.hlsUrl;
  const videoId = modalVideo?.id;
  ...
  return (
    <Modal visible={!!modalVideo} transparent={false} animationType="fade">
      <View style={[positionHelpers.fill, { backgroundColor: colors.black }]}>
        {isVideoLoading && <LoaderIndicator />}
        {hasHlsUrl && (
          <>
            <GestureDetector gesture={combinedGesture}>
              <View style={positionHelpers.fill}>
                <Video
                  key={videoId}
                  ref={videoRef}
                  source={{ uri: modalVideo.file.hlsUrl }}
                  ...
                  controls={false}
                  onLoad={(data) => {
                    handleVideoLoadModal(videoId, data?.duration);
                    ...
                    if (videoId && durations[videoId]) {
                      ...
                      setRemainingSeconds(prev => {
                        ...
                        if (Math.abs((prev[videoId] ?? 0) - newRemaining) > 0.25) {
                          return { ...prev, [videoId]: newRemaining };
                        }
                        return prev;
                      });
                      if (currentTime >= duration) {
                        videoRef.current?.seek(0);
                        onVideoRepeat(videoId);
                      }
                    }
                  }}
                />
            ...
            <VideoAbsoluteInfo
              ...
              videoDuration={formatTime(remainingSeconds[videoId] ?? 0)}
            ...
            {showComments && videoId && (
              <CommentSection videoId={videoId} userId={modalVideo.user.id} onClose={() => setShowComments(false)} />
            )}
          </>
        )}
      </View>
...
export default React.memo(FullVideoModal);